...
chai = OpenAI(api_key=INSERT_YOUR_KEY_HERE)

# this function reads the 1st page of a PDF, and returns a guessed JSON with title, author and pubdate based on GPT-4-Turbo suggestions
def rename_pdf_based_on_title(pdf_path):
    ...
    # Open the PDF and read its metadata
...
# this function sends the first page extract to OpenAI and gives specific instructions on how to format the results (good enough for me) but you can easily change the instructions here. GPT-3.5-Turbo is good enough.
        ...
        response = chai.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "You will only respond in JSON output, strictly with author, title and pubdate values. "
                    "All strings should follow Camel Capitalisation rules only."
                )},
                ...
                author: Institutional_Acronym OR First_Author_FirstName LastName ONLY_IF_UNABLE_TO_GUESS_PRINT_UNKNOWN 
                title: Title and Subtitle of Report without any special symbols ONLY_IF_UNABLE_TO_GUESS_PRINT_UNKNOWN
                pubdate: YYYY AS_STR_NOT_INT ONLY_IF_UNABLE_TO_GUESS_PRINT_PUBDATE THEN_PRINT_TIMESTAMP_INSTEAD YYYYMMDDHHMMSS
